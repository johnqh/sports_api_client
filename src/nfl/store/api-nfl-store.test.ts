/**
 * Tests for ApiNflStore
 */

import { describe, it, expect, beforeEach, vi } from "vitest";
import { createApiNflStore } from "./api-nfl-store";
import { DEFAULT_CACHE_TTL } from "../../utils/cache-utils";

describe("ApiNflStore", () => {
  let store: ReturnType<typeof createApiNflStore>;
  ...
  beforeEach(() => {
    ...
    store = createApiNflStore(mockStorage);
  ...
  describe("setters", () => {
    ...
    it("should set keyed data (games)", () => {
      ...
      store.getState().setGames("nfl-2024", games);
      const state = store.getState();

      expect(state.games.size).toBe(1);
      expect(state.games.get("nfl-2024")?.data).toEqual(games);
    });
  });
